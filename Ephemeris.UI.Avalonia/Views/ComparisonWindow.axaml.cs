...
// TODO: wire in ResearchWorkspaceWindow — add "Open Comparison Mode" button when that window is created.
...
/// </para>
/// <para>
/// Create via <c>new ComparisonWindow(sourceVm).Show()</c> from
/// <c>ResearchWorkspaceWindow</c> (or any other window that holds a <see cref="WorkspaceViewModel"/>).
/// </para>
/// </remarks>
public partial class ComparisonWindow : Window
{
    ...
    /// Creates a default workspace so the designer / previewer can open the window.
    /// </summary>
    public ComparisonWindow() : this(new WorkspaceViewModel()) { }

    /// <summary>
    /// Initialises the comparison window using <paramref name="sourceVm"/> as the baseline.
    /// </summary>
    /// <param name="sourceVm">
    /// Existing workspace whose time and location seed both the baseline and simulation panels.
    /// </param>
    public ComparisonWindow(WorkspaceViewModel sourceVm)
    {
        InitializeComponent();
        ...
        _baselineGl = new SkyGlControl(_vm.Baseline.SkyView)
        {
            HorizontalAlignment = HorizontalAlignment.Stretch,
            VerticalAlignment   = VerticalAlignment.Stretch,
        };
        _simulationGl = new SkyGlControl(_vm.Simulation.SkyView)
        {
            HorizontalAlignment = HorizontalAlignment.Stretch,
            VerticalAlignment   = VerticalAlignment.Stretch,
        };
        ...
        // Initialise header controls from baseline view-model
        SyncHeaderFromVm();
        ...
        StepBackBtn.Click  += (_, _) => _vm.Baseline.SkyView.StepBackCommand.Execute(null);
        PlayBtn.Click      += (_, _) =>
        {
            _vm.Baseline.SkyView.PlayPauseCommand.Execute(null);
            _vm.Baseline.Playing = _vm.Baseline.SkyView.Playing;
        };
        StepFwdBtn.Click   += (_, _) => _vm.Baseline.SkyView.StepForwardCommand.Execute(null);
        StepLastBtn.Click  += (_, _) => _vm.Baseline.SimTime = _vm.Baseline.SimTime.AddDays(30);
        NowBtn.Click       += (_, _) =>
        {
            _vm.Baseline.SkyView.ResetToNowCommand.Execute(null);
            _vm.Baseline.SimTime = _vm.Baseline.SkyView.SimTime;
        };

        // Simulation controls
        FreezeMotionCheck.IsCheckedChanged += (_, _) =>
            _vm.Simulation.Simulation.MotionFrozen = FreezeMotionCheck.IsChecked == true;

        SunAltOffsetPicker.ValueChanged += (_, _) =>
        {
            if (SunAltOffsetPicker.Value is { } v)
                _vm.Simulation.Simulation.SunAltitudeOffsetDegrees = (double)v;
        };

        ExtendDaylightPicker.ValueChanged += (_, _) =>
        {
            if (ExtendDaylightPicker.Value is { } v)
                _vm.Simulation.Simulation.ExtendDaylightHours = (double)v;
        };

        ResetSimBtn.Click += (_, _) => _vm.ResetSimulationCommand.Execute(null);

        // Mouse drag on each panel
    ...
    private void OnBaselinePointerMoved(object? sender, PointerEventArgs e)
    {
        ...
        _vm.Baseline.SkyView.Yaw   = (_vm.Baseline.SkyView.Yaw   + dx * 0.4f + 360f) % 360f;
        _vm.Baseline.SkyView.Pitch = Math.Clamp(_vm.Baseline.SkyView.Pitch - dy * 0.3f, -10f, 90f);
        _lastMouseBaseline = pos;
    }

    private void OnBaselinePointerWheel(object? sender, PointerWheelEventArgs e) =>
        _vm.Baseline.SkyView.FovDeg = Math.Clamp(
            _vm.Baseline.SkyView.FovDeg - (float)e.Delta.Y * 2f, 10f, 170f);
    ...
    private void OnSimPointerMoved(object? sender, PointerEventArgs e)
    {
        ...
        _vm.Simulation.SkyView.Yaw   = (_vm.Simulation.SkyView.Yaw   + dx * 0.4f + 360f) % 360f;
        _vm.Simulation.SkyView.Pitch = Math.Clamp(_vm.Simulation.SkyView.Pitch - dy * 0.3f, -10f, 90f);
        _lastMouseSimulation = pos;
    }

    private void OnSimPointerWheel(object? sender, PointerWheelEventArgs e) =>
        _vm.Simulation.SkyView.FovDeg = Math.Clamp(
            _vm.Simulation.SkyView.FovDeg - (float)e.Delta.Y * 2f, 10f, 170f);
    ...
    private void OnKeyDown(object? sender, KeyEventArgs e)
    {
        switch (e.Key)
        {
            case Key.Space: _vm.Baseline.SkyView.PlayPauseCommand.Execute(null); break;
            case Key.Left:  _vm.Baseline.SkyView.StepBackCommand.Execute(null);  break;
            case Key.Right: _vm.Baseline.SkyView.StepForwardCommand.Execute(null); break;
            case Key.F:     _vm.Baseline.SkyView.ResetToNowCommand.Execute(null); break;
        }
    }
}
